...
async function interactive(seed?: ParsedFromExample): Promise<Answers> {
	...
	const wantCf = await withQuit((ctx) =>
		confirm(
			{
				message:
					"Set Cloudflare API token and account ID now? (Optional locally if you use `wrangler login`; CI and non-interactive deploys usually need a token.)",
				default: false,
			},
			ctx,
		),
...
function afterWriteHints(answers: Answers) {
	...
	if (looksIncomplete(answers.apiToken) || looksIncomplete(answers.accountId)) {
		console.log(
			pc.dim(
				"Deploy: add CLOUDFLARE_API_TOKEN (and account ID) for CI/secrets, or run `bunx wrangler login` locally — Wrangler uses either.",
			),
		);
	}
}
