...
- **`bun run deploy:execute`** — Loads **`.env.production`** via root `package.json` (`--env-file`). Runs **`scripts#sync-secrets:prod`**, **`scripts#pre-deploy`** (queues / checks; **`bootstrapCircularCrossScriptDOBindings`** only **logs** cyclic cross-script DO graphs), **`build:prod`**, **`^deploy`**, then the web app **`wrangler deploy`**. Each **`durable-objects/*#deploy`** runs **`packages/scripts/src/deploy-cyclic-cross-worker.ts`**: **dry-run** both Workers in any **2-node** `script_name` cycle; if OK, **one** full deploy per package; if not, **phased** deploy only from **pipeline primary** (default: lexicographically larger Worker `name`, override **`PIPELINE_PRIMARY_SCRIPT`**). **`cyclic-do-status`** in **`packages/scripts`** prints JSON. If Cloudflare still reports missing secrets after a successful sync, run `turbo run sync-secrets:prod --filter=scripts -- --force` or fix values in `.env.production`.
